...
def amoc_curve(time, y_score, ts_ids, k_fold_label=None, resolution=500):
    ...
    * In the current version, thresholds are drawn from a uniform distribution between the minimum
    ...
    # number of sets to split the data in
    n_test_rounds = 3
    # split the indices into n_test_folds parts
    n_rows_total = int(time.shape[0])  # int conversion so that indices are integers
    ...
    for k, k_fold in enumerate(range(n_test_rounds)):
        # TODO change names of variables from folds to rounds / sets / splits

        # extract start and end index for all arrays
        start_index, end_index = row_indices_parts[k]
        # extract the sub arrays for this fold
        ts_ids_kth = ts_ids[start_index:end_index]
        time_kth = time[start_index:end_index]
        y_true_kth = y_true[start_index:end_index]
        y_score_kth = y_score[start_index:end_index]
...
def plot_amoc(time_list, y_score_list, ts_id_list, k_fold_label_list, model_labels, evaluation_path, conf_stds=1.0, time_unit=None, xlim_max=5):
    ...
    for plot_nr in range(3):  # plot both unit-unit and unit-log version
        ...
        for c in range(number_of_curves):
            ...
            FPR_mean_plotted = FPR_mean[time_to_detection <= xlim_max]
            global_min = min(FPR_mean_plotted.min(), global_min)
        # cosmetics
...
#
# TODO nicely working AMOC implementation using FOLDS, not ROUNDS -> no confidence bounds if only one fold used
# def amoc_curve(time, y_score, ts_ids, k_fold_label=None, resolution=500):
#     """ Compute Activity Monitoring Operating Characteristic (AMOC) [1]
#
#     The Activity Monitoring Operating Characteristic (AMOC) plots "false-alarms", measured in
